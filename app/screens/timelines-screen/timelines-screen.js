...
//   route: TimelineRouteProp<"timelines">
// };
export const TimelinesScreen = observer(function TimelinesScreen({ navigation, route: { params }, }) {
    ...
    const openTimeline = (id, title) => {
        navigation.navigate("timeline", { id, title });
    };
    const renderItem = ({ item: { title, id, description } }) => (<List.Item title={title} key={id} onPress={() => openTimeline(id, title)} description={description} left={(props) => <List.Icon {...props} icon="folder"/>}/>);
    const renderList = () => {
        const timelines = timelineStore.getTimelinesArray();
        if (!timelines)
            return null;
        return (<FlatList data={timelineStore.getTimelinesArray()} renderItem={renderItem} keyExtractor={(item) => item.id.toString()}/>);
    };
    const emptyState = () => {
        return (<View style={styles.emptyStateWrapper}>
        <EmptyState title="Empty in timelines" description="Start by creating a timeline and it will show up here" icon="timeline-plus-outline"/>
...
//# sourceMappingURL=timelines-screen.js.map