...
export const TimelineScreen = observer(function TimelineScreen() {
    ...
    const showDeleteAlert = useCallback(() => {
        Alert.alert("Delete timeline", "Do you really want to delete the timeline and all of the events?", [
            {
                text: "Cancel",
    ...
    useFocusEffect(useCallback(() => {
        ...
        const editTimeline = async (payload) => {
    ...
    const renderEventList = () => {
        const events = timeline.events
            ...
            .map((event) => (<List.Item key={event.id} onPress={() => openEvent(event.id)} title={event.title} description={formatDateYear(new Date(event.date))}/>));
...
//# sourceMappingURL=timeline-screen.js.map