 ...
 */
export const AddTimelineForm = ({ errorText, onSubmit }) => {
    ...
    React.useLayoutEffect(() => {
        const localSubmit = (data /* FIXME: this type */) => {
            const updatedData = {
                title: data.title,
    ...
    return (<View>
     ...
     autoCorrect={false} label="Title" left={<TextInput.Icon name="format-title"/>} right={errors.title && <TextInput.Icon name="alert-circle" color={error}/>} disabled={formState.isSubmitting} onBlur={onBlur} onChangeText={(text) => onChange(text)} error={!!errors.title} value={value}/>
      ...
      <Controller control={control} name="description" render={({ onChange, onBlur, value }) => (<TextInput disabled={formState.isSubmitting} onBlur={onBlur} left={<TextInput.Icon name="script-text-outline"/>} onChangeText={(text) => onChange(text)} label="Description" spellCheck={true} error={!!errors.description} value={value}/>)}/>

      <Text>{errorText || ""}</Text>
    </View>);
};
//# sourceMappingURL=addTimelineForm.js.map